...
<kml xmlns="http://earth.google.com/kml/2.0">
    <Document>
        ...
        <Style id="item-info-balloon">
            <BalloonStyle><text>< ![CDATA[ <div class="geolocation_balloon"><div class="geolocation_balloon_thumbnail">$[description]</div><div class="geolocation_balloon_body"><div class="geolocation_balloon_title">$[namewithlink]</div><p class="geolocation_balloon_description">$[Snippet]</p></div></div>]]></text></BalloonStyle>
        </Style>
        <?php
        foreach (loop('item') as $item):
            ...
            <Placemark>
                <name>
                    <?php echo xml_escape(metadata('item', 'display_title', array('no_escape' => true))); ?>
                </name>
                <namewithlink>
                    <?php echo xml_escape(link_to_item(metadata('item', array('Dublin Core', 'Title')), array('class' => 'view-item text-dark'))); ?>
                </namewithlink>
                <Snippet maxLines="2">
                    <?php echo xml_escape(metadata('item', array('Item Type Metadata', 'Provenance/Provenience'), array('snippet' => 150))); ?>
                </Snippet>
                <description>
                    <?php
                ...
                <collection>
                    <?php echo xml_escape(metadata('item', 'Collection Name')); ?>
                </collection>
                <Point>
                    <coordinates>
                ...
                <?php if ($location['address']): ?>
                    <address>
                        <?php echo xml_escape($location['address']); ?>
                    </address>
                <?php endif; ?>
            </Placemark>
        <?php endforeach; ?>
